...
- **优先使用** 模组定义的 `BackgroundColor`
- **无颜色时** 通过 FNV-1a 哈希从 HSL 色盘分配统一的颜色
- **有图标时** 首次进出群系时从图标中心区域采样主色调（惰性缓存）
...
| 配置项 | 默认值 | 说明 |
|--------|:------:|------|
| 扫描新群系 | 关 | 启用后需**重载模组**才会执行扫描；关闭时已注册的群系仍会继续显示 |
| 配色 | 关 | 为新增的群系自动分配标题颜色（需同时启用扫描） |
